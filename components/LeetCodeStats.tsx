...
function createHeatmap(calendar: string | null | undefined) {
  ...
  return Array.from({ length: HEATMAP_WEEKS * DAYS_PER_WEEK }, (_, index) => {
...
export default async function LeetCodeStats() {
    ...
    return (
      <section className="pb-24 pt-10 sm:pb-32 sm:pt-14" aria-labelledby="leetcode-heading">
        <div className="leetcode-header">
          ...
          <h2 id="leetcode-heading" className="mt-3 text-2xl font-medium tracking-tight text-cp-text">Coding activity</h2>
          <p className="mt-3 text-sm text-cp-subtext">Stats are temporarily unavailable. Please check back shortly.</p>
        </div>
      </section>
    );
  }
  ...
  const solved = user.submitStatsGlobal.acSubmissionNum.reduce<Record<string, number>>((totals, stat) => {
    ...
    <section className="pb-24 pt-10 sm:pb-32 sm:pt-14" aria-labelledby="leetcode-heading">
      <div>
        <div className="leetcode-header flex flex-wrap items-end justify-between gap-5">
          <div>
            <p className="font-mono text-xs uppercase tracking-[0.18em] text-cp-peach">LeetCode</p>
            <h2 id="leetcode-heading" className="mt-3 text-2xl font-medium tracking-tight text-cp-text sm:text-3xl">Trying to solve one problem a day :)</h2>
          </div>
          <a
        ...
        <div className="leetcode-dashboard mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_14rem] lg:gap-12">
          <div>
            <div className="pb-2">
              <div className="leetcode-heatmap" aria-label="LeetCode submission activity over the last year">
                {heatmap.map((day) => (
                  <span
                    key={day.dateKey}
            ...
            <div className="mt-4 flex items-center justify-between text-xs text-cp-subtext">
              ...
              <span className="flex items-center gap-1.5">
                Less
                {[0, 1, 3, 7].map((count) => <i key={count} className="leetcode-key" style={{ backgroundColor: heatColor(count) }} />)}
                More
              </span>
            </div>
          </div>
          ...
          <dl className="grid grid-cols-2 gap-x-6 gap-y-6 lg:grid-cols-1 lg:gap-y-5">
            <div><dt className="font-mono text-[11px] uppercase tracking-wider text-cp-subtext">Global rank</dt><dd className="mt-1 text-xl font-medium text-cp-text">#{formatNumber(user.profile.ranking)}</dd></div>
            <div><dt className="font-mono text-[11px] uppercase tracking-wider text-cp-subtext">Streak</dt><dd className="mt-1 text-xl font-medium text-cp-green">{formatNumber(user.userCalendar?.streak)} <span className="text-xs text-cp-subtext">days</span></dd></div>
            <div><dt className="font-mono text-[11px] uppercase tracking-wider text-cp-subtext">Active days</dt><dd className="mt-1 text-xl font-medium text-cp-text">{formatNumber(user.userCalendar?.totalActiveDays)}</dd></div>
          </dl>
        </div>

        <div className="leetcode-solved mt-8 grid grid-cols-2 gap-x-6 gap-y-6 sm:grid-cols-4">
          <div><p className="font-mono text-xs uppercase tracking-wider text-cp-subtext">Solved</p><p className="mt-2 text-3xl font-medium text-cp-text">{formatNumber(solved.All)}</p></div>
          <div><p className="font-mono text-xs uppercase tracking-wider text-cp-subtext">Easy</p><p className="mt-2 text-3xl font-medium text-cp-green">{formatNumber(solved.Easy)}</p></div>
          <div><p className="font-mono text-xs uppercase tracking-wider text-cp-subtext">Medium</p><p className="mt-2 text-3xl font-medium text-cp-peach">{formatNumber(solved.Medium)}</p></div>
          <div><p className="font-mono text-xs uppercase tracking-wider text-cp-subtext">Hard</p><p className="mt-2 text-3xl font-medium text-cp-maroon">{formatNumber(solved.Hard)}</p></div>
        </div>
      </div>
    </section>
  );
